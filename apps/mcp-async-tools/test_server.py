"""Comprehensive test suite for mcp-async-tools server."""
...
class TestIntegration:
    ...
    async def test_async_wait(self):
        result = await server.call_tool("async_wait", {"seconds": 0.1})
        assert "Wait" in result[0].text or "0." in result[0].text
