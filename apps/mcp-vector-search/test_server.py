"""Comprehensive test suite for mcp-vector-search server."""
...
class TestIntegration:
    ...
    async def test_add_and_search(self):
        await server.call_tool("add_vector", {"text": "Python programming"})
        result = await server.call_tool("search_vectors", {"query": "Python"})
        assert len(result) > 0
