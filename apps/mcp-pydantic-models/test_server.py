"""Comprehensive test suite for mcp-pydantic-models server."""
...
class TestIntegration:
    ...
    async def test_validate_model(self):
        result = await server.call_tool("validate_user", {"name": "Alice", "age": 30, "email": "alice@example.com"})
        assert len(result) > 0
        assert "Valid user" in result[0].text
